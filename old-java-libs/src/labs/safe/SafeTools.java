...
public class SafeTools
{
	...
	public static void checkBufferStartEnd(byte[] buffer, int start, int end, final String bufferName)
	{
		...
		if (end < start)
			throw new IllegalArgumentException(bufferName + " end index must be greater than " + bufferName
					+ " start index" + ", start: " + start + ", end: " + end);
	}
	...
	public static void checkEqualBufferLength(byte[] buffer1, byte[] buffer2, final String bufferName1,
			final String bufferName2)
	{
