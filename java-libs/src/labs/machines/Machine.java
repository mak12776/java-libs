...
public class Machine
{
	...
	{
		NULL_BASE_INST_POINTER, NULL_BUFFERS_INDEX,

		INVALID_BASE_INST_POINTER, INVALID_INST_POINTER,

		INVALID_POINTERS_INDEX, INVALID_BUFFERS_INDEX, INVALID_DATA_BUFFER_INDEX,
	...
	{
		return (short) (((instBuffer[ip++] & 0xFF) << (8 * 1)) | ((instBuffer[ip++] & 0xFF) << (8 * 0)));
	}

	private int readInt()
	{
		return ((instBuffer[ip++] & 0xFF) << (8 * 3)) | ((instBuffer[ip++] & 0xFF) << (8 * 2))
				| ((instBuffer[ip++] & 0xFF) << (8 * 1)) | ((instBuffer[ip++] & 0xFF) << (8 * 0));
	}
