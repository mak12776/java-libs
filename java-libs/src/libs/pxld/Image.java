...
public class Image implements Picture
{
	...
	public void drawView(int x, int y, View view)
	{
		...
		for (int yOffset = y * xWidth, yViewOffset = view.yStart() * xWidth, yEnd = (y + view.height()) * xWidth; 
				yOffset < yEnd; yOffset += xWidth, yViewOffset += xWidth)
		{
			for (int xOffset = x * colorWidth, xViewOffset = view.xStart() * colorWidth, xEnd = (x + view.width()) * colorWidth; 
					xOffset < xEnd; x += colorWidth, xViewOffset += colorWidth)
			{
				for (int c = 0; c < colorWidth; c += 1)
				{
