...
public class BufferMachine
{
	...
	private void checkBaseInstructionPointer()
	{
		if ((bip < 0) || (bip >= buffers.length))
			throw new RuntimeError(ErrorType.INVALID_BASE_INSTRUCTION_POINTER, String.valueOf(bip));
	}
	...
	{
		if (instBuffer == null)
			throw new RuntimeError(ErrorType.NULL_INSTRUCTION_POINTER, String.valueOf(index));
	}
	...
	/*
	 * symbol size description IM8/32 8/32 immediate PI 32 pointers index BI 32
	 * buffers index
	 *
	 * PI:PI 32 + 32 address at: buffers [pointers[PI]] [pointers[PI]] BI:BI 32 + 32
	 * address at: buffers [buffer index] [buffer index]
	 *
	 * PI:BI 32 + 32 address at: ... BI:PI 32 + 32 address at: ...
	 *
	...
	public void run() throws RuntimeError
	{
		...
		while (ip < instBuffer.length)
		{
			...
			{
			default:
				throw new RuntimeError(ErrorType.UNKNOWN_INSTRUCTION, Integer.toHexString(inst));
